...
from lumaflux.color.transfer import CORPUS_PEAK_PQ
...
def test_decoder_identity_at_init():
    ...
    x = torch.rand(2, 3, 16, 16) * CORPUS_PEAK_PQ
...
def test_decoder_caps_output_at_corpus_mastering_peak():
    dec = RQSToneFieldDecoder(latent_channels=4, num_knots=8)
    x = torch.ones(1, 3, 8, 8)
    z = torch.randn(1, 4, 2, 2)
    out = dec(x, z)["hdr"]
    assert out.max().item() <= CORPUS_PEAK_PQ + 1e-6
