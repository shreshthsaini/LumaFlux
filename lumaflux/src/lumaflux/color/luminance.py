...
from .transfer import bt709_eotf, pq_eotf
...
def sdr_to_linear2020(x_sdr: torch.Tensor) -> torch.Tensor:
    ...
    linearize with the BT.709 inverse OETF and rotate primaries.
    """
    lin709 = bt709_eotf(x_sdr)
    return rgb709_to_rgb2020(lin709).clamp(0.0, 1.0)


def pq_to_linear2020(x_pq: torch.Tensor) -> torch.Tensor:
    """PQ BT.2020 signal [0,1] -> normalized linear light [0,1]."""
    return pq_eotf(x_pq)
