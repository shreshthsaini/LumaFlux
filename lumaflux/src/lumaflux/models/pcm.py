...
class PerceptualConnector(nn.Module):
    """C_perc: projects SigLIP tokens to the MM-DiT channel size."""

    def __init__(self, siglip_dim: int, model_dim: int) -> None:
        super().__init__()
...
class PCMModulator(nn.Module):
    def __init__(self, model_dim: int) -> None:
        ...
        self.mlp = nn.Sequential(nn.SiLU(), nn.Linear(model_dim, 2 * model_dim))
