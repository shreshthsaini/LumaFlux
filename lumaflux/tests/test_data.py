...
def synthetic_root(tmp_path_factory):
    ...
    subprocess.run(
        [sys.executable, str(SCRIPTS / "make_synthetic_data.py"),
...
@pytest.fixture(scope="module")
def curated(synthetic_root, tmp_path_factory):
    out = tmp_path_factory.mktemp("curated")
    cfg = CurationConfig(
        sources=[
            SourceSpec(name="pgc_demo", path=str(synthetic_root / "pgc"),
                       category="pgc", format="pq2020"),
            SourceSpec(name="ugc_demo", path=str(synthetic_root / "ugc"),
