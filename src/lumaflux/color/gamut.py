...
# Linear BT.2020 -> linear BT.709. Report ITU-R BT.2407-0 Sections
# 2.1 through 2.3 require transfer-function decoding, a linear primary
# matrix, and transfer-function encoding in that order.
...
# The report prints four decimal places. Restore the exact neutral-axis
# invariant that each row sums to one before deriving the inverse.
M_2020_TO_709 = M_2020_TO_709 / M_2020_TO_709.sum(dim=1, keepdim=True)

# Keep the inverse numerically exact for this implementation. The resulting
# coefficients round to the BT.709 -> BT.2020 values in BT.2087 Table 2.
M_709_TO_2020 = torch.linalg.inv(M_2020_TO_709.to(torch.float64)).to(torch.float32)
...
# Mathematical inverse of the matrix in Report ITU-R BT.2446-1 Section
# 6.1.3, Equation 3. Its rounded values are printed in Section 6.1.5,
# Equation 13. Deriving it here makes the implemented pair exact.
M_XYZ_TO_2020 = torch.linalg.inv(M_2020_TO_XYZ.to(torch.float64)).to(torch.float32)
...
def gamut_expand_709_to_2020(x: torch.Tensor, channel_dim: int = -3) -> torch.Tensor:
    """Invert the linear matrix part of the selected BT.2407 conversion.

    Hard clipping in :func:`gamut_compress_2020_to_709` is intentionally not
    invertible. Values that were inside BT.709 before clipping round-trip
    through this matrix exactly up to floating-point error.
    """
    return rgb709_to_rgb2020(x, channel_dim)
