...
def pu21_decode(encoded: torch.Tensor) -> torch.Tensor:
    """Invert :func:`pu21_encode` back to absolute luminance in cd/m^2."""
    p = _PU21_P
    ratio = (encoded.clamp(min=0.0) / p[6] + p[5]).pow(1.0 / p[4])
    powered_luminance = ((ratio - p[0]) / (p[1] - p[2] * ratio)).clamp(min=0.0)
    return powered_luminance.pow(1.0 / p[3]).clamp(0.005, 10_000.0)


PU21_PEAK = pu21_encode(torch.tensor(10_000.0, dtype=torch.float64)).item()
