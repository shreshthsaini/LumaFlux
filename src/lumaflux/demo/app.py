...
def build_demo(pipe: LumaFluxPipeline, mult: int):
    ...
    def convert(image: np.ndarray, num_steps: int, seed: int):
        if image is None:
        ...
        # Plot the spline parameters that were applied to the integrated latent.
        with torch.no_grad():
            params = out["spline_params"]
            widths = params["widths"]
            heights = params["heights"]
            derivs = params["derivs"]
            xs = torch.linspace(0, 1, 256).unsqueeze(0)
            ys = rqs_apply(xs, widths[:1], heights[:1], derivs[:1])
        import matplotlib
    ...
    with gr.Blocks(title="LumaFlux SDR->HDR") as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                steps = gr.Slider(1, 60, value=8, step=1, label="ODE steps")
