...
from .video_decode import FFMPEG, FFMPEG_THREADS, ffmpeg_available, ffmpeg_env
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(raw_in, "wb") as f:
            for fr in frames:
                f.write(fr.tobytes())
        mp4 = td / "out.mp4"
        common = [str(FFMPEG), "-y", "-loglevel", "error"]
        subprocess.run(
            common + ["-f", "rawvideo", "-pix_fmt", "rgb24", "-s", f"{w}x{h}", "-r", "24",
                      "-i", str(raw_in), "-c:v", "libx264", "-crf", str(crf),
                      "-threads", str(FFMPEG_THREADS), "-pix_fmt", "yuv420p", str(mp4)],
            env=ffmpeg_env(),
            check=True,
        )
        out = subprocess.run(
            common + ["-threads", str(FFMPEG_THREADS), "-i", str(mp4),
                      "-threads", str(FFMPEG_THREADS), "-f", "rawvideo",
                      "-pix_fmt", "rgb24", "-"],
            env=ffmpeg_env(),
            check=True,
