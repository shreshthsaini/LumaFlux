...
from ..color.transfer import CORPUS_PEAK_NITS
from .adapters import wrap_flux_blocks
from .lora import inject_backbone_lora
...
class LumaFluxModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        rqs_mode: str = "monotone",
        lora_mode: str = "none",
        lora_rank: int | None = None,
        time_layer_modulation: bool = True,
    ) -> None:
        super().__init__()
        ...
        self.enable_pcm = enable_pcm
        self.context: dict = {"active": False}
        self.physical = PhysicalEncoder(phys_channels, stats_dim, num_bands)
        self.perc_connector = PerceptualConnector(siglip_dim, self.inner_dim, bottleneck)
        self.ctx_connector = PerceptualConnector(siglip_dim, self.context_dim, bottleneck)
        ...
        self.lora_modules = inject_backbone_lora(
            transformer,
            rank=rank if lora_rank is None else lora_rank,
            mode=lora_mode,
        )
        adapters_enabled = any((enable_pga, enable_pcm, enable_coupler))
        self.modulation = (
            TimestepLayerModulation(
                num_layers,
                modulation_hidden,
                adaptive=time_layer_modulation,
            )
            if adapters_enabled
            else None
        )
        ...
        if rqs_mode not in {"monotone", "linear", "disabled"}:
            raise ValueError(f"Unknown RQS mode: {rqs_mode}")
        self.rqs = (
            None
            if rqs_mode == "disabled"
            else RQSToneFieldDecoder(
        ...
        if not enable_pcm:
            self.perc_connector.requires_grad_(False)
    ...
    def decode_latent(self, z: torch.Tensor) -> torch.Tensor:
        """Decode a latent to an unconformed PQ/BT.2020 VAE signal."""
    ...
    def prepare_condition(self, x_sdr: torch.Tensor) -> dict:
        ...
        perc_tokens = self.perc_connector(sig_tokens) if self.enable_pcm else None
    ...
    def velocity(
        ...
        ctx.update(
            ...
            num_context_tokens=cond["ctx_tokens"].shape[1],
            grid_hw=grid_hw,
        )
    ...
    def decode_hdr(
        ...
        """Final latent -> fixed-peak PQ/BT.2020 HDR via VAE + RQS tone field.
        ...
        if self.rqs is None:
            return {"hdr": x_out, "vae_out": x_out}
        out = self.rqs(x_out, z, spline_params=spline_params)
        out["vae_out"] = x_out
        return out
    ...
    def load_trainable_state_dict(
        ...
        if strict and (missing or unexpected):
            ...
            raise RuntimeError("Invalid adapter checkpoint, " + "; ".join(details))
